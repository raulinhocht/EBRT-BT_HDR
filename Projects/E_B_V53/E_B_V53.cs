...
namespace VMS.TPS
{
    public class Script
    {
        ...
        {
            // Obtener la dosis prescrita total (Ejemplo: 85 Gy para cáncer de cervix)
            double dosisPrescrita = 86.0; // Valor de prescripción total (Gy)

            // Calcular dosis de referencia para CTV (dref)
            double dosisTotalEBRT = eqd2Total["PTV+CTV"]; // EQD2 acumulado de EBRT
            double dref = (dosisPrescrita - dosisTotalEBRT) / 5.0;

            // Modificar esta línea para tomar solo los primeros 5 planes
            var plans = course.BrachyPlanSetups?
                .OrderBy(p => p.Id)
                .Take(5)  // <- Aquí está el cambio clave
                .ToList();
                    ...
                    Dictionary<string, (double aimValue, double limitValue, string type, string aimText, string limitText)> constraints, double totalDosisEBRT, int totalFraccionesBT)
        {
            ...
            if (btDosesPerFraction != null && btDosesPerFraction.Any())
            {
                ...
                {
                    Orientation = Orientation.Horizontal, // Horizontal en lugar de Vertical
                    Margin = new Thickness(10),
                    VerticalAlignment = VerticalAlignment.Top
                };

                // Gráfico 1: EQD2 por fracción (ajustado a 60% del ancho original)
                var eqd2PerFxChart = new GroupBox
                {
                    Header = "EQD2 por Fracción",
                    Margin = new Thickness(5),
                    Padding = new Thickness(5),
                    Width = 580, // Tamaño reducido
                    Content = CreateEQD2PerFractionChart(btDosesPerFraction)
                };

                // Gráfico 2: EQD2 Total (ajustado a 60% del ancho original)
                var totalChart = new GroupBox
                {
                    Header = "EQD2 Total con Límites",
                    Margin = new Thickness(5),
                    Padding = new Thickness(5),
                    Width = 400, // Tamaño reducido
                    Content = CreateEQD2ChartWithLimits(eqd2Total, constraints)
                };

                chartPanel.Children.Add(eqd2PerFxChart);
                chartPanel.Children.Add(totalChart);
        ...
        //---------------------------++++++++++++++++
        private Canvas CreateEQD2PerFractionChart(Dictionary<string, List<double>> btDosesPerFraction)
        {
            const double margin = 50;
            const double barWidth = 25; // Más delgadas (antes 40)
            const double spacing = 10;
            const double groupSpacing = 30; // Espacio entre grupos de fracciones
            const double chartHeight = 400;
            const double chartWidth = 750; // Más ancho para acomodar todas las estructuras
            const double targetEQD2 = 6.0;
            const double tolerance = 0.1;
            ...
            // Configurar escala
            double maxDose = btDosesPerFraction.Max(kvp => kvp.Value.DefaultIfEmpty().Max()) * 1.3;
            double scaleFactor = (chartHeight - 2 * margin) / maxDose;

            // Dibujar ejes (igual que antes)
            ...
            {
                ["HR-CTV"] = Brushes.SteelBlue,
                ["Recto-HDR"] = Brushes.ForestGreen,
                ["Sigma-HDR"] = Brushes.Purple,
                ["Vejiga-HDR"] = Brushes.Orange
            };

            // Procesar cada fracción (máx 5)
            for (int fx = 0; fx < 5; fx++)
            {
                double xStart = margin + spacing + (fx * (groupSpacing + (barWidth * btDosesPerFraction.Count)));

                // Dibujar todas las estructuras para esta fracción
                int structIndex = 0;
                foreach (var kvp in btDosesPerFraction)
                {
                    if (fx >= kvp.Value.Count) continue;

                    double xPos = xStart + (structIndex * barWidth);
                    double eqd2 = kvp.Value[fx];
                    double barHeight = eqd2 * scaleFactor;
                    ...
                    Canvas.SetLeft(valueLabel, xPos + (barWidth / 2) - 10);
                    Canvas.SetTop(valueLabel, chartHeight - margin - (barHeight / 2) - 10);
                    canvas.Children.Add(valueLabel);
                ...
                Canvas.SetLeft(fxLabel, xStart + ((btDosesPerFraction.Count * barWidth) / 2) - 10);
                Canvas.SetTop(fxLabel, chartHeight - margin + 5);
                canvas.Children.Add(fxLabel);
            }

            // Leyenda (actualizada para incluir todas las estructuras)
            DrawMultiStructureLegend(canvas, margin, structureColors);
            return canvas;
        }

        private void DrawMultiStructureLegend(Canvas canvas, double margin, Dictionary<string, Brush> structureColors)
        {
            var legend = new StackPanel
            {
                Orientation = Orientation.Horizontal,
            ...
            foreach (var kvp in structureColors)
            {
                var panel = new StackPanel { Orientation = Orientation.Horizontal, Margin = new Thickness(2) };
                panel.Children.Add(new Rectangle
                {
                ...
                {
                    Text = kvp.Key.Replace("-HDR", ""),
                    FontSize = 10
                });
                legend.Children.Add(panel);
            }

            Canvas.SetLeft(legend, margin + 10);
            Canvas.SetTop(legend, margin + 10);
            canvas.Children.Add(legend);
        }
           ...
           Dictionary<string, (double aimValue, double limitValue, string type, string aimText, string limitText)> constraints)
        {
            ...
            */
            ...
            foreach (var item in eqd2Total)
            {
                ...
                var valueLabel = new TextBlock
                {
                    Text = item.Value.ToString("F1") + " Gy",  // Agregamos " Gy" para claridad
                    FontSize = 11,
                    Foreground = Brushes.Black,
                    FontWeight = FontWeights.Bold,
                    HorizontalAlignment = HorizontalAlignment.Center,
                ...
                // Posición horizontal: centro de la barra (xPos + mitad del ancho - ajuste)
                Canvas.SetLeft(valueLabel, xPos + (barWidth / 2) - 15);
                // Posición vertical: mitad de la altura de la barra
