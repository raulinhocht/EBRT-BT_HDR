...
namespace VMS.TPS
{
    public class Script
    {
        ...
        public void Execute(ScriptContext context)
        {
            ...
            sb.AppendLine(" PRUEBA V28 - Resumen Consolidado de Datos con EQD2, Ajuste por Tiempo y Evaluación");
            sb.AppendLine("=====================================================================================");
            ...
            foreach (Course curso in context.Patient.Courses)
            {
                ...
                if (curso.Id.Contains(EBRT) || curso.Id.Contains("EBRT") || curso.Id.Contains("CERVIX"))
                {
                    ...
                    {
                        if (planext.ApprovalStatus == PlanSetupApprovalStatus.Completed)
                        {
        ...
        // Parámetros para el ajuste por tiempo:-------------------------------------------------------------ttttttttttttttt
        double totalTime = 35.0;  // Valor fijo de ejemplo (se puede modificar)
        double Tdelay = 21.0;
        double k = 0.6;

        private double TimeAdjustment(double totalTime, double Tdelay, double k)
        {
            return totalTime > Tdelay? k * (totalTime - Tdelay) : 0;
        }
}
    //{
