...
namespace VMS.TPS
{
    public class Script
    {
        ...
        {
            sb.AppendLine("\n========================== SECCIÓN EBRT ==========================");
            // En ProcessEBRTCourse (después de sb.AppendLine):
            sb.AppendLine($"║   EBRT: {totalDosisEBRT:F2} Gy en {totalFraccionesEBRT} fx   ║");
            ...
            foreach (var plan in course.ExternalPlanSetups)
            {
                if (IsPlanApproved(plan))
                {
                    ...
                    sb.AppendLine($"\nPlan: {plan.Id}");
                    //sb.AppendLine($"Dosis total: {totalDosisEBRT:F2} Gy en {totalFraccionesEBRT} fracciones");
                    sb.AppendLine("--------------------------------------------------------------------------------------------------");
        ...
        private void ShowReportWindow(StringBuilder sb)
        {
            ...
            // Procesar texto directamente sin ApplyFormatting
            textBlock.Inlines.AddRange(ProcessTextForColors(sb.ToString()));
        ...
        private IEnumerable<Inline> ProcessTextForColors(string text)
        {
            ...
            foreach (var line in lines)
            {
                ...
                // Asignar colores según contenido
                ...
                else if (line.Contains("Gy") || line.Contains("EQD2"))
                {
