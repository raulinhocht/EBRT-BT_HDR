...
namespace VMS.TPS
{
    public class Script
    {
        ...
        private const double BrachyDoseReference = 5.0;    // 5 Gy de referencia
        ...
        public void Execute(ScriptContext context)
        {
            ...
            ShowReportWindow(reportBuilder, btDosesData, eqd2Totals, constraints);
        }
        //----------------------------------------------------------------------------------------------------------------------
        ...
        ref double totalDosisEBRT, ref int totalFraccionesEBRT, Dictionary<string, double> eqd2Total)
        {
            ...
            foreach (var (structureId, volume, alphaBeta, key) in GetStructureDefinitions())
            {
                ...
                if (IsValidDose(doseAtVolume))
                {
                    ...
                    reportBuilder.AppendLine($"| {structureId,-10} | {doseAtVolume / 100,-7:F2} | {eqd2,-7:F2} |");
                    eqd2Total[key] += eqd2;
    ...
    ref double totalDosisBT, ref int totalFraccionesBT, Dictionary<string, double> eqd2Total)
        {
            ...
            reportBuilder.AppendLine("\n====================== SECCIÓN HDR-BT ======================")
                        ...
                        .AppendLine("------------------------------------------------------------------------------------------------------------------------")
                        .AppendLine("| Estructura       | Métrica       | Fx #1    | Fx #2    | Fx #3    | Fx #4    | Fx #5    | Total     | Validación    |")
                        .AppendLine("------------------------------------------------------------------------------------------------------------------------");
        ...
        ("Sigma-HDR", targetVolumeAbs2, alphaBetaOAR, "Sigma", "D2cc [Gy]")
            ...
            foreach (var structure in structures)
            {
                ...
                reportBuilder.Append($" {totalDose,-7:F2} | {validationResult,-13} |\n");
                ...
                reportBuilder.Append($" {totalEQD2,-7:F2} | {"",-13} |\n")
                            .AppendLine("------------------------------------------------------------------------------------------------------------------------");
            }
        ...
        /*
        private string ValidateBrachyDose(double dose)
        {
            if (dose < 0) return "No calculable";
            if (dose < 5) return "No válido";
            if (dose >= 5 && dose <= 10) return "Aceptable";
            return "Excesivo";
        }
        */
        private string ValidateBrachyDose(double dose)
        {
            if (dose == 0) return "-"; // Para fracciones no administradas

            double lowerLimit = BrachyDoseReference * (1 - BrachyDoseTolerance);
            double upperLimit = BrachyDoseReference * (1 + BrachyDoseTolerance);

            if (dose >= lowerLimit && dose <= upperLimit)
            {
                return "<span style='color:green;font-weight:bold'>Cumple</span>";
            }
            else
            {
                return "<span style='color:red;font-weight:bold'>No cumple</span>";
            }
        }
        ...
        private Canvas CreateEQD2PerFractionChart(List<double> eqd2Values)
        {
            ...
            const double targetEQD2 = 5.0; // Ajustado a 5 Gy como referencia
        ...
        private void ShowRecalculationDialog()
        {
            ...
            btnCalculate.Click += (s, e) =>
            {
                try
                {
                    ...
                    // Ejemplo de parámetros - deberías obtener estos del contexto real
                    var results = RecalculateRemainingFractions(
                        prescribedTotalDose: 45.0,
                        originalDosePerFraction: 1.8,
                        totalPlannedFractions: 25,
                        deliveredFractions: 25 - remainingFractions,
                        treatmentStartDate: DateTime.Now.AddDays(-20),
                        originalEndDate: DateTime.Now.AddDays(13),
                        newEndDate: DateTime.Now.AddDays(13 + additionalDays));

                    // Mostrar resultados
                    resultText.Text = $"Nueva dosis/fracción: {results.NewDosePerFraction:F2} Gy\n" +
