...
namespace VMS.TPS
{
    public class Script
    {
        ...
        {
            // Validación inicial simplificada
        ...
        // Helper methods extracted from main method
        private static IEnumerable<(string structureId, double volume, double alphaBeta, string key)> GetStructureDefinitions()
        {
            return new[] {
        ...
        ref double totalDosisBT, ref int totalFraccionesBT, Dictionary<string, double> eqd2Total)
        {
            ...
            // Ensure we always show 5 fractions (pad with zeros if needed)
            int maxFractions = Math.Max(5, plans.Count);
            ...
            foreach (var structure in structures)
            {
                ...
                // Process physical doses (show 0 for missing fractions)
                var physicalDosesResult = ProcessPhysicalDoses(plans, structureId, defaultVolume, maxFractions);
                ...
                // Process EQD2 values (show 0 for missing fractions)
                var eqd2ValuesResult = ProcessEQD2Values(plans, structureId, defaultVolume, alphaBeta, maxFractions);
        ...
        private Canvas CreateEQD2PerFractionChart(List<double> eqd2Values)
        {
            ...
            const double targetEQD2 = 8.0; // Ajustado a 8 Gy como referencia
