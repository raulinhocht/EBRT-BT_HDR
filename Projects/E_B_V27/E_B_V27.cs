    ...
    public class Script
    {
            ...
            Dictionary<string, (double aimValue, double limitValue, string type, string aimText, string limitText)> constraints)
        {
            foreach (var item in eqd2Total)
            {
                ...
                {
                    // Si no se tiene definición para alguna estructura, asumimos que no cumple
                    return false;
                }
            }
            return true;
        ...
        public void Execute(ScriptContext context)
        {
            ...
            {
                if (curso.Id.Contains(EBRT))
                {
                    var planAprobado = curso.ExternalPlanSetups.FirstOrDefault(plan => plan.ApprovalStatus == PlanSetupApprovalStatus.Completed);
                    MessageBox.Show("73", planAprobado.Id);
                ...
                else if (curso.Id.Contains(HDR_BT))
                {
            ...
            sb.AppendLine(" PRUEBA V27 - Resumen Consolidado de Datos con EQD2, Ajuste por Tiempo y Evaluación");
            sb.AppendLine("=====================================================================================");
            ...
            sb.AppendLine("-------------------------------------------------------------------------------------");
            sb.AppendLine($" EBRT: {totalFraccionesEBRT} fracciones   |   Dosis Prescrita: {totalDosisEBRT} Gy");
            sb.AppendLine($" BT: {totalFraccionesBT} fracciones   |   Dosis Prescrita: {totalDosisBT} Gy");
            sb.AppendLine("=====================================================================================\n");
            ...
            foreach (Course curso in context.Patient.Courses)
            {
                ...
                if (curso.Id.Contains(EBRT) || curso.Id.Contains("EBRT") || curso.Id.Contains("CERVIX"))
                {
                    sb.AppendLine("\n========================== SECCIÓN EBRT ==========================");
        ...
        private void ProcessEBRTPlan(
            ...
            Dictionary<string, double> eqd2Total)
        {
            // Parámetros para el ajuste por tiempo:
            double totalTime = 35.0;  // Valor fijo de ejemplo (se puede modificar)
            double Tdelay = 21.0;
            double k = 0.6;
        ...
        private double ProcessEQD2ForPlan(PlanSetup plan, string structureId, double targetVolume, double alphaBeta,
            double totalTime, double Tdelay, double k)
        {
        ...
        private double CalculateBEDWithTimeAdjustment(double dosePerFraction, double fractions, double alphaBeta,
            double totalTime, double Tdelay, double k)
        {
